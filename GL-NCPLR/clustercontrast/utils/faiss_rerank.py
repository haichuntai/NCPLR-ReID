...
def compute_jaccard_distance(target_features, k1=20, k2=6, print_flag=True, search_option=0, use_float16=False, neighbor_dist=None):
    end = time.time()
    if print_flag:
    ...
    #     V_qe = np.zeros_like(V, dtype=mat_type)
    #     for i in range(N):
    #         inds_k2 = np.where(neighbor_dist[i] <= 0.2)[0]
    #         # V_qe[i,:] = np.mean(V[inds_k2,:], axis=0)
    #         dist_i = (1 - neighbor_dist[i][inds_k2])[:, np.newaxis]
    #         V_qe[i,:] = np.sum((V[inds_k2,:] * dist_i) / dist_i, axis=0)
    #     V = V_qe
    #     del V_qe
